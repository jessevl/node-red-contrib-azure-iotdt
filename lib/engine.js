 ...
 * @param {{ parameters: Object }} context 
...
module.exports = async function (context, device, measurements, timestamp) {
    ...
    try {
        ...
        console.log('[HTTP] Sending telemetry for device: ' + device.deviceId);
        ...
        for(var measurement in measurements){
            ...
            console.log('[HTTP] Sending telemetry for sensor: ' + measurement + ' with value: ' + measurements[measurement]);
            // Send the message
...
async function getDeviceConnectionString(context, device, measurements) {
    ...
    try {
        console.log('[HTTP] Requesting device connectionstring');
        ...
        if (!getDevice[0]) {
            console.log("[HTTP] Device does not exist. Creating new device.");
...
async function createDevice(context, deviceId, measurements){
    ...
    console.log("[HTTP] Created new device with id: " + createDevice);
    return createDevice;
}
...
async function getRootSpace(context){
    ...
    console.log("[HTTP] Root space object: " + JSON.stringify(rootSpace));
    console.log("[HTTP] Root Space id is: " + rootSpace[0].id);
...
async function getDigitalTwinToken(context) {
    if (dtToken.token && dtToken.created && ((Date.now() - dtToken.created) < 60*60*1000)){
        console.log('Getting Digital Twin token from cache');
        ...
        try {
            console.log('[HTTP] Requesting new Digital Twin token');
